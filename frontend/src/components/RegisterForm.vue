    ...
    <div class="card">
      ...
      <form class="form" @submit.prevent="handleSubmit">
        <div class="form-grid">
          ...
          <div class="section">
            <label for="name">Nome:
              <input id="name" type="text" v-model="form.name" @input="validateField('name')" class="input" placeholder="Escribe o teu nome"/>
              <span class="error" v-if="errors.name">{{ errors.name }}</span>
            </label>

            <label for="surname">Apelidos:
              <input id="surname" type="text" v-model="form.surname" @input="validateField('surname')" class="input" placeholder="Escribe os teus apelidos"/>
              <span class="error" v-if="errors.surname">{{ errors.surname }}</span>
            </label>

            <label for="dni">DNI:
              <input id="dni" type="text" v-model="form.dni" @input="validateField('dni')" class="input" placeholder="12345678X"/>
              <span class="error" v-if="errors.dni">{{ errors.dni }}</span>
            </label>

            <label for="birthDate">Data de nacemento:
              <input id="birthDate" type="date" v-model="form.birthDate" @input="validateField('birthDate')" class="input"/>
          ...
          <div class="section">
            <label for="address">Enderezo:
              <input id="address" type="text" v-model="form.address" @input="validateField('address')" class="input" placeholder="Rúa, avenida, etc."/>
              <span class="error" v-if="errors.address">{{ errors.address }}</span>
            </label>

            <label for="zipCode">Código Postal:
              <input id="zipCode" type="text" v-model="form.zipCode" @input="validateField('zipCode')" class="input" placeholder="XXXXX"/>
              <span class="error" v-if="errors.zipCode">{{ errors.zipCode }}</span>
            </label>

            <label for="town">Localidade:
              <input id="town" type="text" v-model="form.town" @input="validateField('town')" class="input" placeholder="Cidade ou vila"/>
              <span class="error" v-if="errors.town">{{ errors.town }}</span>
            </label>

            <label for="province">Provincia:
              <input id="province" type="text" v-model="form.province" @input="validateField('province')" class="input" placeholder="Provincia"/>
          ...
          <div class="section">
            <label for="telephone">Teléfono:
              <input id="telephone" type="tel" v-model="form.telephone" @input="validateField('telephone')" class="input" placeholder="Número fixo"/>
              <span class="error" v-if="errors.telephone">{{ errors.telephone }}</span>
            </label>

            <label for="mobilePhone">Teléfono móbil:
              <input id="mobilePhone" type="tel" v-model="form.mobilePhone" @input="validateField('mobilePhone')" class="input" placeholder="Número móbil"/>
          ...
          <div class="section">
            <label for="email">Email:
              <input id="email" type="email" v-model="form.email" @input="validateField('email')" class="input" placeholder="usuario@exemplo.com"/>
              <span class="error" v-if="errors.email">{{ errors.email }}</span>
            </label>

            <label for="password">Contrasinal:
              <input id="password" type="password" v-model="form.password" @input="validateField('password')" class="input" placeholder="********"/>
          ...
          <div class="section">
            <label for="bankName">Entidade bancaria:
              <input id="bankName" type="text" v-model="form.bankName" @input="validateField('bankName')" class="input" placeholder="Nome do banco"/>
              <span class="error" v-if="errors.bankName">{{ errors.bankName }}</span>
            </label>

            <label for="iban">IBAN:
              <input id="iban" type="text" v-model="form.iban" @input="validateField('iban')" class="input" placeholder="ES00 0000 0000 0000 0000 0000"/>
...
function validateField(field: string) {
  switch (field) {
    ...
    case 'province':
      ...
      break
    case 'telephone':
      errors.telephone = form.telephone === '' ? '' : (/^[0-9]+$/.test(form.telephone) ? '' : 'O teléfono debe ser numérico.');
      break
    case 'mobilePhone':
      errors.mobilePhone = form.mobilePhone === '' ? '' : (/^[0-9]+$/.test(form.mobilePhone) ? '' : 'O teléfono debe ser numérico.');
      break
    case 'email':
      errors.email = form.email === '' ? '' : (/^[0-9]+$/.test(form.email) ? '' : 'O email non é correcto.');
      break
    case 'password':
    ...
    case 'bankName':
      ...
      break
    case 'iban':
      errors.iban = form.iban === '' ? '' : (/^[0-9]+$/.test(form.iban) ? '' : 'O IBAN non é correcto');
      break
